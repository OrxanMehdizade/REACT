...
export let reducer = (state, action) => {
    switch(action.type){
        case 'ADD':
            ...
            newArr[0] = {...newArr[0], products:[...newArr[0].products, action.payload]}
            console.log(newArr)
            return {...state, goodsArray:newArr}
        case 'DELETE':
            let newArr2 = [...state.goodsArray]
            let arrOne = newArr2[0].products.filter((item) => item.product_name !== action.payload)
            newArr2[0] = {...newArr2[0], products:[...arrOne]}
            console.log(newArr2)
            return {...state, goodsArray:newArr2}
